...
Mobile landscape media query clashing with tablet media query  |     Moving the media query from bottom of style sheet to below mobile queries helped. Targeting problematic elements with more specific css using classes such as .heading for the game title did solve some fo the issues, but left some unsolvable ones.  Adding max height 500px to the mobile orientation landscape media query stopped the clash with the tablet media query and resolved the issue.                              | 404 page scoring 95 on best practice lighthouse testing | Console logging an error from js script, removed script tag from 404 html to resolve problem. 
